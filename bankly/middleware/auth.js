...
function authUser(req, res, next) {
  try {
    ...
    if (token) {
      // BUG #1 FOUND: WE ARE NOT VERIFYING TOKEN LEGITIMACY WITH THE CODE BELOW:
      // let payload = jwt.decode(token);
      // req.curr_username = payload.username;
      // req.curr_admin = payload.admin;

      // BUG #1: FIXED VERIFICATION BELOW:
      jwt.verify(token, SECRET_KEY, (err, verified) => {
        if (err) {
          return err;
        }
        req.curr_username = verified.username;
        req.curr_admin = verified.admin;
      });
    }
    return next();
  } catch (err) {
